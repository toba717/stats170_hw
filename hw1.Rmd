...
log_data = diff(retails_electronic_train_log) # mean is centered around zero
plot(log_data) # now it is a mean of zero and has a constant variance
acf(log_data)
pacf(log_data) # partial autocorrelation
```
...
lines(retails_elecronic_test, col = "yellow", lty = 10)
```
...
```{r}
preditc_furniture = predict(hw_furniture, n.ahead = 20)
plot(hw_furniture, preditc_furniture)
lines(retails_furniture_test, col = "blue")
```