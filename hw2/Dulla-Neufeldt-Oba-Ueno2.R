...
# FORMAT THIS BETTER - WHERE NUMBERS COME FROM?
...
# T-Test Statistics for Parameters -- One is FALSE?
...
sdresiduals　<-　(mlr.star.residuals-resmean)/ressd
...
res_model <- arima(sdresiduals.ts, order = c(0, 1, 1), seas = list(order = c(0, 1, 1), 12))
...
Box.test(resid(res_model), lag = 12, type = "Ljung")

# FIX CODE STARTING HERE.
library(nlme)
modelgls <- gls(y.star~x1.star+x2.star, correlation=corARMA(c(-0.5161), q=1))
summary(modelgls)

acf(residuals(modelgls, type = "normalized")) # NOT WHITE NOISE RESIDUAL
#plot(y=residuals(modelgls,type="normalized"), type="l",
    # ylab="uncorrelated residuals",
     #xlab="Time",
     #main="Example of uncorrelated residuals")
#abline(h=0)

acf(modelgls$residuals)
pacf(modelgls$residuals)

df_future = data.frame(x1.star = retails_hobby_test, 
                       x2.star = retails_hobby_test)
